  ...
  communication_msgs__msg__TaskAllocation__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    communication_msgs__msg__TaskAllocation * data =
      (communication_msgs__msg__TaskAllocation *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!communication_msgs__msg__TaskAllocation__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          communication_msgs__msg__TaskAllocation__fini(&output->data[i]);
        }
        return false;
      }
    }
